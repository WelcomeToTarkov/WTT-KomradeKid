...
namespace UnityGB
{
    public class Emulator : EmulatorBase
    {
        ...
        private long TICKS_PER_FRAME = (long)(Stopwatch.Frequency / FRAMES_PER_SECOND);
        ...
        public EmulatorState currentState = EmulatorState.Stopped;

        public Emulator(IVideoOutput video, IAudioOutput audio = null, ISaveMemory saveMemory = null)
            : base(video, audio, saveMemory)
        {
            // Initialize the emulator state
            InitializeEmulatorState();
        }
        private void InitializeEmulatorState()
        {
        ...
        public void Start()
        {
            if (currentState == EmulatorState.Stopped)
            {
        ...
        public void Stop()
        {
            if (currentState == EmulatorState.Running || currentState == EmulatorState.Paused)
            {
				if (game != null)
				{
                    game.Save();
                ...
                currentState = EmulatorState.Stopped;
				stopwatch.Stop();
				ResetState();
            }
        }
        ...
        {
            if (currentState != EmulatorState.Running) return;
            ...
            {
                lastFrameTime += TICKS_PER_FRAME;
                UpdateModel(true);
                Video.SetPixels(pixels);

                // Adjust lastFrameTime to prevent lagging
                if (stopwatch.ElapsedTicks > lastFrameTime + TICKS_PER_FRAME * 2)
                {
                    lastFrameTime = stopwatch.ElapsedTicks;
                }
            }
            ...
            {
                // Calculate the sleep time to keep the framerate capped
                int sleepTime = (int)((TICKS_PER_FRAME - elapsedTime) * 1000 / FREQUENCY);
                if (sleepTime > 0)
                {
                    // Use Task.Delay instead of Thread.Sleep for better responsiveness
