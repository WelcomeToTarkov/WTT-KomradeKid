...
import type { GameboyCartridgeProps } from "./mod";
...
export class CustomItemService {
    ...
    public postDBLoad(): void {
        ...
        this.createCartridgeClones();
        const customCartridges = this.loadCustomCartridgesFromDirectory();
        for (const cartridgeId in customCartridges) {
            const cartridgeConfig = customCartridges[cartridgeId];
            this.processTraders(cartridgeConfig, cartridgeId);
        }
    }


    private createCartridgeClones() {
        const customCartridges = this.loadCustomCartridgesFromDirectory();
        // Process and clone cartridges
        for (const cartridgeId in customCartridges) {
            const cartridgeConfig = customCartridges[cartridgeId];
            const baseCartridgeId = "670fffeedf87b6d12c7c78bf";
            const baseCartridgeParent = "66f17b4cb59dbccbf12990e6";
            const electronicsHandbookParentId = "5b47574386f77428ca22b2ef";

            const cartridgeVariant: NewItemFromCloneDetails = {
                itemTplToClone: baseCartridgeId,
                overrideProperties: {
                    Prefab: {
                        path: cartridgeConfig.Prefabpath,
                        rcid: ""
                    },
                    RomName: cartridgeConfig.RomName,
                    CartridgeImage: cartridgeConfig.CartridgeImage
                } as GameboyCartridgeProps,
                parentId: baseCartridgeParent,
                newId: cartridgeId,
                fleaPriceRoubles: cartridgeConfig.fleaPriceRoubles,
                handbookPriceRoubles: cartridgeConfig.handbookPriceRoubles,
                handbookParentId: electronicsHandbookParentId,
                locales: cartridgeConfig.locales
            };

            if (this.Instance.debug) {
                console.log(`Cloned Cartridge ID: ${cartridgeId}`);
                console.log(`Rom Name: ${cartridgeConfig.RomName}`);
            }
            this.Instance.customItem.createItemFromClone(cartridgeVariant);

            this.processStaticLootContainers(cartridgeConfig, cartridgeId);
            this.processModSlots(cartridgeConfig, [baseCartridgeId], cartridgeId);
            this.processInventorySlots(cartridgeConfig, cartridgeId);
            this.processMasterySections(cartridgeConfig, cartridgeId);
            this.processWeaponPresets(cartridgeConfig, cartridgeId);
            this.addtoHallofFame(cartridgeConfig, cartridgeId);
            this.addtoSpecialSlots(cartridgeConfig, cartridgeId);
        }
   ...
   */
    ...
    private processTraders(
        ...
        // Iterate over each trader in the provided configuration
        for (const [traderId, traderData] of Object.entries(itemConfig.traders)) {
            ...
            if (!trader) {
                continue; // Skip this trader if it's not found
            }

            // Iterate over each barter scheme key in the traderData
            for (const [schemeKey, schemeData] of Object.entries(traderData)) {
                // Skip if the key is not a barter scheme (e.g., "_settings")
                if (schemeKey.endsWith('_settings')) {
                    continue;
                }

                const settingsKey = `${schemeKey}_settings`;
                const settings = traderData[settingsKey];

                // Create a new item to add to the trader's assort items
                const newItem = {
                    _id: schemeKey,
                    _tpl: itemId,
                    parentId: "hideout",
                    ...
                    upd: {
                        ...
                        StackObjectsCount: settings.stackObjectsCount
                    }
                };
                ...
                if (this.Instance.debug) {
                    console.log(`Added item ${itemId} to trader ${traderId} with scheme key ${schemeKey}`);
                }

                // Process the barter schemes for the item
                trader.assort.barter_scheme[schemeKey] = [];

                // Map through the scheme data to prepare barter scheme items
                const barterSchemeItems = schemeData.map(scheme => {
                    const count = scheme.count;
                    // Look up the template (tpl) in ItemMap first, fallback to the scheme's _tpl if not found.
                    const tplFromMap = ItemMap[scheme._tpl] || scheme._tpl;
                    const finalTpl = tplFromMap || scheme._tpl; // Ensure a fallback
                    return {
                        count: count,
                        _tpl: finalTpl
                    };
                ...
                // Push all the items required for this scheme as a single array
                trader.assort.barter_scheme[schemeKey].push(barterSchemeItems);
                if (this.Instance.debug) {
                    console.log(`Added barter scheme to trader ${traderId} for item ${itemId} with scheme key ${schemeKey}:`, barterSchemeItems);
                }
                // Set the loyalty level for the item in the trader's assort
                trader.assort.loyal_level_items[schemeKey] = settings.loyalLevel;
            }
        }
    }
    ...
    private addtoSpecialSlots(itemConfig: ConfigItem[string], itemId: string) {
        if (itemConfig.addtoSpecialSlots) {
            console.log(`Attempting to add item '${itemId}' to special slots.`);
            ...
            if (!items) {
                console.error("Items object is undefined, cannot add to special slots.");
                return;
            }
    
            console.log("Items object successfully retrieved.");
    
            const pocketTypes = Object.values(items).filter(item => item._parent === "557596e64bdc2dc2118b4571");
            console.log(`Found ${pocketTypes.length} pocket types matching the criteria.`);
    
            for (const pockets of pocketTypes) {
                console.log(`Processing pocket: ${pockets._id}`);
    
                for (const slot of pockets._props.Slots) {
                    console.log(`Checking slot: ${slot._name} for item ID: ${itemId}`);
    
                    // Check if the itemId is not already in the filter
                    if (!slot._props.filters[0].Filter.includes(itemId)) {
                        slot._props.filters[0].Filter.push(itemId);
                        console.log(`Item ID '${itemId}' added to slot filter of '${slot._name}' in pocket '${pockets._id}'.`);
                    } else {
                        console.log(`Item ID '${itemId}' is already present in slot filter of '${slot._name}'. No action taken.`);
                    }
                }
            }
    
            console.log("Finished processing all pockets.");
        } else {
            console.log(`Item '${itemId}' does not require adding to special slots. Skipping.`);
        }
    }
   ...
   */
    ...
    private loadCustomCartridgesFromDirectory(): any[] {
        const cartridgeFiles = fs.readdirSync(path.join(__dirname, "../db/cartridges"));
        const cartridges: any[] = [];

        const combinedCartridgeConfig: any = {};

        cartridgeFiles.forEach((file) => {
            const cartridgePath = path.join(__dirname, "../db/cartridges", file);
            const cartridgeFileContents = fs.readFileSync(cartridgePath, "utf-8");

            // Parse the entire JSON file
            const cartridgeData = JSON.parse(cartridgeFileContents);

            Object.assign(combinedCartridgeConfig, cartridgeData);
        });

        return combinedCartridgeConfig;
    }
